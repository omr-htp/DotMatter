    ...
    Action<string, string, string, string> onStateChanged)
{
    ...
    private void PublishDerivedHueSaturation(string id, string endpointId, ushort? colorX, ushort? colorY)
    {
        ...
        var hueSaturation = ColorConversion.XYToHueSat(colorX.Value, colorY.Value);
        registry.Update(id, device => { device.Hue = hueSaturation.Hue; device.Saturation = hueSaturation.Saturation; });
        onStateChanged(id, endpointId, "hue", hueSaturation.Hue.ToString());
        onStateChanged(id, endpointId, "saturation", hueSaturation.Saturation.ToString());
    }
