...
internal sealed class SubscriptionCoordinator(
    ...
    Action<string, int, int> trackSubscriptionShape,
    Action<string, ReportDataAction> processAttributeReports,
    Action<string, IReadOnlyList<MatterEventReport>> processEventReports)
{
    private const ushort SubscriptionMinIntervalSeconds = 1;
    private const ushort AttributeSubscriptionMaxIntervalSeconds = 30;
    private const ushort EventOnlySubscriptionMaxIntervalSeconds = 5;

    public async Task StartAsync(string id, ISession session, CancellationToken ct = default)
    {
        var device = registry.Get(id);
        var paths = BuildAttributePaths(device);
        var eventPaths = BuildEventPaths(device);

        if (paths.Count == 0 && eventPaths.Count == 0)
        {
            trackSubscriptionShape(id, 0, 0);
            log.LogInformation("[MATTER] {Id}: no subscribable attributes or events discovered", id);
            return;
        }

        trackSubscriptionShape(id, paths.Count, eventPaths.Count);

        var maxIntervalSeconds = GetSubscriptionMaxIntervalSeconds(
            paths.Count,
            eventPaths.Count,
            device?.SupportsCluster(SwitchCluster.ClusterId) == true);
        ...
        try
        {
            var subscription = await Subscription.CreateMultiAsync(
                ...
                paths,
                eventPaths,
                minInterval: SubscriptionMinIntervalSeconds,
            ...
            lastSubscriptionReport[id] = DateTime.UtcNow;
            subscriptionStaleThresholds[id] = GetSubscriptionStaleThreshold(subscription.MaxIntervalSeconds);
    ...
    private static List<AttributePath> BuildAttributePaths(DeviceInfo? device)
    {
        ...
        if (device?.SupportsCluster(SwitchCluster.ClusterId) == true)
        {
            paths.Add(new(
                device.EndpointFor(SwitchCluster.ClusterId),
                SwitchCluster.ClusterId,
                SwitchCluster.Attributes.CurrentPosition));
        }
    ...
    private static ushort GetSubscriptionMaxIntervalSeconds(int attributePathCount, int eventPathCount)
        => GetSubscriptionMaxIntervalSeconds(attributePathCount, eventPathCount, hasSwitchCluster: false);

    private static ushort GetSubscriptionMaxIntervalSeconds(int attributePathCount, int eventPathCount, bool hasSwitchCluster)
        => (attributePathCount == 0 && eventPathCount > 0) || hasSwitchCluster
            ? EventOnlySubscriptionMaxIntervalSeconds
            : AttributeSubscriptionMaxIntervalSeconds;

    private static TimeSpan GetSubscriptionStaleThreshold(ushort maxIntervalSeconds)
        => TimeSpan.FromSeconds(Math.Max(5, (maxIntervalSeconds * 2) + 2));
}
