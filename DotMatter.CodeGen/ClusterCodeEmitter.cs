...
{
    private sealed record MethodParameterEntry(string Parameter, bool IsOptional);
        ...
        HashSet<string> structNames)
    {
        ...
        foreach (var s in cluster.Structs)
        {
            ...
            sb.AppendLine($"        => Read{name}(tlv, null);");
            sb.AppendLine();
            sb.AppendLine($"    private static {name} Read{name}(MatterTLV tlv, int? tag)");
            sb.AppendLine("    {");
            sb.AppendLine($"        var value = new {name}();");
            sb.AppendLine("        tlv.OpenStructure(tag);");
        ...
        Dictionary<string, StructModel> structMap)
    {
        ...
        foreach (var cmd in clientCmds)
        {
            ...
            if (cmd.Fields.Count == 0)
            {
                ...
                {
                    sb.AppendLine($"    public Task<InvokeResponse> {methodName}(CancellationToken ct = default, ushort timedTimeoutMs = 5000)");
                    sb.AppendLine($"        => InvokeTimedCommandAsync(0x{cmd.Id:X4}, timedTimeoutMs: timedTimeoutMs, ct: ct);");
                }
                else
                {
            ...
            else
            {
                ...
                var paramEntries = new List<MethodParameterEntry>();
                ...
                foreach (var f in cmd.Fields)
                {
                    ...
                    paramEntries.Add(new MethodParameterEntry($"{csType} {paramName}{defaultVal}", f.Optional));
                ...
                var paramList = paramEntries
                    .Where(p => !p.IsOptional).Select(p => p.Parameter)
                    .Concat(paramEntries.Where(p => p.IsOptional).Select(p => p.Parameter))
                    .Append("CancellationToken ct = default")
                    .Concat(cmd.MustUseTimedInvoke ? ["ushort timedTimeoutMs = 5000"] : [])
                    .ToList();
                ...
                else if (cmd.MustUseTimedInvoke && tlvLines.Count == 1)
                {
                    sb.AppendLine($"        => InvokeTimedCommandAsync(0x{cmd.Id:X4}, tlv => {{ {tlvLines[0]} }}, timedTimeoutMs: timedTimeoutMs, ct: ct);");
                }
                else if (cmd.MustUseTimedInvoke)
                {
                    sb.AppendLine($"        => InvokeTimedCommandAsync(0x{cmd.Id:X4}, tlv =>");
                    sb.AppendLine("        {");
                    ...
                    sb.AppendLine("        }, timedTimeoutMs: timedTimeoutMs, ct: ct);");
                }
        ...
        HashSet<string> structNames)
    {
        ...
        foreach (var attr in cluster.Attributes)
        {
            ...
            else if (structNames.Contains(baseCsType))
            {
                ...
                sb.AppendLine($"        => ReadStructAttributeAsync(0x{attr.Id:X4}, raw => Read{baseCsType}(raw, 2), ct);");
            }
        ...
        HashSet<string> structNames)
    {
        ...
        if (csType.EndsWith('?') || !Naming.IsValueType(csType))
        {
    ...
    static string TrimNullable(string csType)
        => csType.EndsWith('?') ? csType[..^1] : csType;
