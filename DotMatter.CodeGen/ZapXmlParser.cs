...
static class ZapXmlParser
{
    ...
    static void ParseZapFormat(XElement root, List<ClusterModel> results)
    {
        ...
        var topLevelOnlyClusterIds = topEnums.Keys
            .Concat(topBitmaps.Keys)
            .Concat(topStructs.Keys)
            .Distinct()
            .Where(clusterId => !clustersById.ContainsKey(clusterId))
            .OrderBy(clusterId => clusterId);

        foreach (var clusterId in topLevelOnlyClusterIds)
        {
            var cluster = new ClusterModel
            {
                Id = clusterId,
                Name = $"Cluster 0x{clusterId:X4}",
                Revision = 1,
            };
    ...
    static CommandModel? ParseZapCommand(XElement el)
    {
        ...
        var cmd = new CommandModel
        {
            ...
            MustUseTimedInvoke = el.Attribute("mustUseTimedInvoke")?.Value == "true",
        ...
        if (commands != null)
        {
            foreach (var c in commands.Elements().Where(e => e.Name.LocalName == "command"))
            {
                var cmd = new CommandModel
                {
                    ...
                    MustUseTimedInvoke = c.Attribute("mustUseTimedInvoke")?.Value == "true"
                        || c.Attribute("timed")?.Value == "true",
