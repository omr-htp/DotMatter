...
public class FabricDiskStorage : IFabricStorageProvider
{
    ...
    {
        var normalizedFabricName = NormalizeFabricName(fabricName);
        var allFiles = Directory.GetFiles(GetFullPath(normalizedFabricName));

        var fabric = new Fabric()
        {
            FabricName = normalizedFabricName,
        };
        ...
        var allDirectories = Directory.GetDirectories(GetFullPath(normalizedFabricName));
    ...
    {
        var fabricName = NormalizeFabricName(fabric.FabricName);
        var fabricDir = GetFullPath(fabricName);
        ...
        {
            var nodeDirectoryPath = GetFullPath(fabricName, node.NodeId);
    ...
    {
        return Path.Combine(_rootDirectory, NormalizeFabricName(fabricName));
    }

    private string GetFullPath(string fabricName, BigInteger nodeId)
    {
        return Path.Combine(GetFullPath(fabricName), nodeId.ToString());
    }

    private static string NormalizeFabricName(string fabricName)
    {
