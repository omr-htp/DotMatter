...
/// <summary>SPAKE2+ initiator values derived for PASE.</summary>
public sealed record PakeInitiatorValues(BigInteger W0, BigInteger W1, BigInteger XScalar, P256Math.ECPoint X);

/// <summary>SPAKE2+ shared encryption key and confirmation verifiers.</summary>
public sealed record Pake2Result(byte[] Ke, byte[] HAY, byte[] HBX);

/// <summary>CryptographyMethods class.</summary>
public class CryptographyMethods
{
    /// <summary>Crypto_PAKEValues_Initiator.</summary>
    public static PakeInitiatorValues Crypto_PAKEValues_Initiator(uint passcode, ushort iterations, byte[] salt)
    {
        ...
        return new PakeInitiatorValues(w0, w1, x, X);
    }

    /// <summary>ECPoint, byte[]).</summary>
    public static Pake2Result Crypto_P2(byte[] contextHash, BigInteger w0, BigInteger w1, BigInteger x, P256Math.ECPoint X, byte[] Y)
    {
    ...
    private static Pake2Result ComputeSecretAndVerifiers(byte[] contextHash, BigInteger w0, P256Math.ECPoint X, byte[] Y, P256Math.ECPoint Z, P256Math.ECPoint V)
    {
        ...
        return new Pake2Result(Ke, hAY, hBX);
    }
