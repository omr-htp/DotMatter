    ...
    ResilientSessionOptions? options = null) : IAsyncDisposable, IDisposable
{
    ...
    /// <summary>
    /// Returns the current CASE session, reconnecting first when needed.
    /// </summary>
    public async Task<CaseSecureSession> GetConnectedSessionAsync(CancellationToken ct = default)
    {
        bool sessionExpired;
        CaseSecureSession? existingSession;
        lock (_transportLock)
        {
            existingSession = _session;
            sessionExpired = existingSession != null
                && (DateTime.UtcNow - _lastActivity) >= _options.SessionIdleTimeout;
        ...
        if (!await ConnectAsync(ct))
        {
            throw new MatterSessionException("Unable to establish a CASE session.");
        }
        ...
        lock (_transportLock)
        {
            if (_session is null)
            {
                throw new MatterSessionException("CASE session establishment completed without creating a session.");
            }
    ...
    /// <summary>
    /// Executes an operation with a connected CASE session and retries after reconnecting if the operation fails.
    /// </summary>
    public async Task<T> UseSessionAsync<T>(Func<CaseSecureSession, Task<T>> action, CancellationToken ct = default)
    {
        ...
        for (int attempt = 1; attempt <= _maxRetries; attempt++)
        {
            ...
            var session = await GetConnectedSessionAsync(ct);
            try
            {
            ...
            {
                lastFailure = ex;
                MatterLog.Warn("[ResilientSession] Operation failed (attempt {0}/{1}): {2}", attempt, _maxRetries, ex.Message);
                CloseTransport();
            }
        }

        throw new MatterSessionException("Failed to execute the Matter operation after reconnect attempts.", lastFailure!);
    }

    /// <summary>
    /// Executes an operation with a connected CASE session and retries after reconnecting if the operation fails.
    /// </summary>
    public async Task UseSessionAsync(Func<CaseSecureSession, Task> action, CancellationToken ct = default)
    {
        ArgumentNullException.ThrowIfNull(action);

        await UseSessionAsync(async session =>
        {
