...
/// <summary>Matter CIE xy coordinates encoded as uint16 values.</summary>
public readonly record struct MatterColorXy(ushort X, ushort Y);

/// <summary>Matter hue and saturation values encoded in the 0-254 range.</summary>
public readonly record struct MatterHueSaturation(byte Hue, byte Saturation);
...
public static class ColorConversion
{
    ...
    /// </summary>
    public static MatterColorXy MiredsToXY(ushort mireds)
    {
    ...
    /// </summary>
    public static MatterColorXy HueSatToXY(byte hue, byte saturation)
    {
    ...
    /// </summary>
    public static MatterHueSaturation XYToHueSat(ushort matterX, ushort matterY)
    {
        ...
        if (cy < 1e-6)
        {
            return new MatterHueSaturation(0, 0);
        }
        ...
        rLin = Math.Max(rLin, 0);
        gLin = Math.Max(gLin, 0);
        bLin = Math.Max(bLin, 0);
        ...
        return new MatterHueSaturation(matterHue, matterSat);
    }

    private static MatterColorXy SrgbToMatterXY(double r, double g, double b)
    {
        ...
        if (sum < 1e-6)
        {
            return new MatterColorXy(0, 0);
        }

        double cx = X / sum;
        double cy = Y / sum;

        return new MatterColorXy(
            (ushort)Math.Clamp(cx * 65535, 0, 65535),
            (ushort)Math.Clamp(cy * 65535, 0, 65535));
    }
}
