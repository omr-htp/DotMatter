...
public static class Extensions
{
    ...
    {
        var friendlyNames = GetFriendlyNames(message.MessagePayload);
        return string.Format("[S: {0}] {1} | {2} | {3} | ack:{4}", message.SessionID, message.MessageCounter, friendlyNames.ProtocolId, friendlyNames.ProtocolOpCode, message.MessagePayload.AcknowledgedMessageCounter);
    }

    /// <summary>DebugInfo.</summary>
    public static string DebugInfo(this MessagePayload message)
    {
        var friendlyNames = GetFriendlyNames(message);
        return string.Format("[E: {0}] {1} | {2} | {3} | {4}", message.ExchangeID, message.ExchangeFlags, message.ExchangeID, friendlyNames.ProtocolId, friendlyNames.ProtocolOpCode);
    }

    private sealed record ProtocolFriendlyNames(string ProtocolId, string ProtocolOpCode);

    private static ProtocolFriendlyNames GetFriendlyNames(MessagePayload messagePayload)
    {
        ...
        return new ProtocolFriendlyNames(protocolName, protocolOpName);
    }
