...
public class CaseSecureSession : ISession
{
    ...
    /// </summary>
    private (ushort sessionId, ushort exchangeId, uint counter, bool reliable, bool initiator) ParseHeader(byte[] raw)
    {
        if (raw.Length < 8)
        {
        ...
        {
            return (sessionId, 0, counter, false, false);
        }
        ...
        return (sessionId, exchangeId, counter, reliable, initiator);
    }
    ...
    /// <inheritdoc />
    public TimeSpan? PeerMrpIdleRetransTimeout { get; set; }

    /// <inheritdoc />
    public TimeSpan? PeerMrpActiveRetransTimeout { get; set; }
