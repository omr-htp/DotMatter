...
public static class CertificateChainVerifier
{
    ...
    sealed class ParsedCert
    {
        ...
        public (string oid, string value)[] IssuerDn = default!;
        public DateTime NotBefore, NotAfter;
        public (string oid, string value)[] SubjectDn = default!;
        ...
        public List<(int tag, ulong value)> SubjectAttrs = [];
    }
    ...
    static ulong GetRequiredSubjectAttribute(ParsedCert cert, int tag, string name)
    {
        ...
        if (attr.tag == 0)
        {
            throw new MatterCertificateException($"NOC missing {name} in subject");
        }
    ...
    static ParsedCert ParseTlvCert(byte[] tlvBytes)
    {
        ...
        tlv.OpenList(3);
        var issuerAttrs = new List<(int tag, ulong value)>();
        while (!tlv.IsEndContainerNext())
        {
            int tag = tlv.PeekTag();
            ulong val = (ulong)tlv.GetUnsignedInt(tag);
            issuerAttrs.Add((tag, val));
        }
        tlv.CloseContainer();
        cert.IssuerDn = [.. issuerAttrs.Select(a => (_dnTagToOid[a.tag], a.value.ToString("X16")))];
        ...
        while (!tlv.IsEndContainerNext())
        {
            int tag = tlv.PeekTag();
            ulong val = (ulong)tlv.GetUnsignedInt(tag);
            cert.SubjectAttrs.Add((tag, val));
        }
        tlv.CloseContainer();
        cert.SubjectDn = [.. cert.SubjectAttrs.Select(a => (_dnTagToOid[a.tag], a.value.ToString("X16")))];
    ...
    static byte[] BuildExtensionsDer(ParsedCert cert)
    {
        ...
        using (w.PushSequence())
        {
            ...
            var (kuByte, unusedBits) = TlvKeyUsageToDer(cert.KeyUsage);
            MatterCertSigner.WriteExtension(w, "2.5.29.15", true,
                MatterCertSigner.BuildKeyUsageDer(kuByte, unusedBits));
    ...
    static (byte derByte, int unusedBits) TlvKeyUsageToDer(ushort tlvFlags)
    {
        byte der = 0;
        int highestBit = -1;
        for (int i = 0; i < 9; i++)
