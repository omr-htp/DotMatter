...
public static class MatterCertSigner
{
        ...
        ECDsa signingKey)
    {
        ...
        var dn = new[] { ("1.3.6.1.4.1.37244.1.4", rcacHex) };
    ...
    public static byte[] SignNoc(
        ...
        ECDsa signingKey)
    {
        var issuerDn = new[] { ("1.3.6.1.4.1.37244.1.4", MatterIdToHex(rcacId)) };
        var subjectDn = new[] {
            ("1.3.6.1.4.1.37244.1.1", MatterIdToHex(nodeId)),
            ("1.3.6.1.4.1.37244.1.5", MatterIdToHex(fabricId))
        };
    ...
    internal static byte[] BuildDerTbs(
        byte[] serialNumber,
        (string oid, string value)[] issuer,
        DateTime notBefore,
        DateTime notAfter,
        (string oid, string value)[] subject,
        byte[] publicKey,
    ...
    /// </summary>
    internal static void WriteName(AsnWriter w, (string oid, string value)[] attrs)
    {
        using (w.PushSequence()) // Name = SEQUENCE
        {
            foreach (var (oid, val) in attrs)
            {
                using (w.PushSetOf()) // RDN = SET
                using (w.PushSequence()) // AttributeTypeAndValue
                {
                    w.WriteObjectIdentifier(oid);
                    w.WriteCharacterString(UniversalTagNumber.UTF8String, val);
                }
            }
        }
    }
