...
| `GET /api/discovery/commissionable` | Supported for lab/operator workflows | Browses currently advertising commissionable Matter devices over mDNS, BLE, or both. |
| `GET /api/discovery/commissionable/resolve` | Supported for lab/operator workflows | Resolves a currently advertising commissionable device by short or long discriminator. |
| `POST /api/commission` | Supported | BLE commissioning for Thread devices, including fabric persistence and follow-up reconnect scheduling. |
| `POST /api/commission/wifi` | Supported | BLE commissioning for Wi-Fi devices when SSID/password are explicitly supplied. |
| Device overview/state/capabilities endpoints | Supported | `/api/devices`, `/api/devices/{id}`, `/api/devices/{id}/state`, and `/api/devices/{id}/capabilities` support controller/UI workflows. |
...
| Device network commissioning endpoints | Supported | Read, scan, add/update, connect, remove, reorder, and interface-enable operations for promoted `NetworkCommissioning` flows. |
| Device group/key/scene endpoints | Supported | Endpoint-aware `Groups`, `GroupKeyManagement`, and `ScenesManagement` query/command workflows. |
| Device fabric/admin endpoints | Supported | Commissioning-window state/open/revoke/complete, fabric inventory, fabric label update, and fabric removal workflows. |
