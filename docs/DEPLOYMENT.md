...
This guide covers deploying `DotMatter.Controller` and the companion `DotMatter.Ui` service to a Raspberry Pi running Debian-based Linux.
...
The companion UI runs as a separate framework-dependent service and does **not** conflict with the controller services:

| Service | Type | Pi Path | Runtime Env File | Default URL |
|---------|------|---------|------------------|-------------|
...
sudo mkdir -p /opt/dot-matter /opt/dot-matter-aot /opt/dot-matter-ui
sudo chown dotmatter:dotmatter /opt/dot-matter /opt/dot-matter-aot /opt/dot-matter-ui
...
sudo cp dot-matter-ui.service /etc/systemd/system/

sudo systemctl daemon-reload
sudo systemctl enable dot-matter dot-matter-aot dot-matter-ui
```

Or run the tracked helper script from the repo root to recreate the **controller**
systemd units, controller env examples, and Samba share include file in one step:

```bash
sudo bash scripts/install-dotmatter-system-files.sh
```

Install `dot-matter-ui.service` separately if you want the companion UI on the Pi; the helper script does not manage the UI service yet.
...
| `/etc/dotmatter/dot-matter-ui.env` | UI service environment overrides |
| `/opt/dot-matter` | Debug deployment directory |
| `/opt/dot-matter-aot` | AOT deployment directory |
| `/opt/dot-matter-ui` | UI deployment directory |
...
- `DotMatter.Ui/DotMatter.Ui.Deploy.local.props` for UI-specific Pi host overrides when you do not want to inherit the controller deploy props
- `/etc/dotmatter/*.env` for host-local runtime configuration
- `dot-matter.service` / `dot-matter-aot.service` / `dot-matter-ui.service` only if you need a different service account or filesystem layout
...
sudo systemctl status dot-matter-ui

# View logs
sudo journalctl -u dot-matter -f
sudo journalctl -u dot-matter-ui -f
