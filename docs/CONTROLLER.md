...
- **Commissionable-device discovery** — browse current commissionable mDNS advertisements with filterable typed results and exact discriminator resolution
- **Network commissioning management** — read, scan, connect, reorder, and network/interface administration routes for promoted `NetworkCommissioning` workflows, including explicit invoke-status metadata for device-side rejections
- **Group management workflows** — query and manage `Groups`, `GroupKeyManagement`, and `ScenesManagement` through typed controller/Core contracts with explicit endpoint-aware validation
- **Switch binding + removal** — creates and reconciles Matter ACL and Binding state for a switch OnOff route
- **Admin / fabric operations** — commissioning-window reads, basic/enhanced window open, revoke, commissioning complete, fabric inventory, fabric label update, and fabric removal
...
- **Companion operator UI** — the separate `DotMatter.Ui` Blazor/Radzen project consumes this REST API for richer lab/test workflows than Scalar alone; see [UI Guide](UI.md)
...
| GET | `/api/discovery/commissionable` | Browse currently advertising commissionable devices over mDNS, BLE, or both |
| GET | `/api/discovery/commissionable/resolve?discriminator={value}` | Resolve one currently advertising commissionable device by short or long discriminator over mDNS, BLE, or both |
| GET | `/api/devices` | List all devices |
| GET | `/api/devices/{id}` | Device details |
| GET | `/api/devices/{id}/capabilities` | Get the capability snapshot used by the Blazor UI for topology, device-type, cluster, and operation-support rendering |
...
| GET | `/api/devices/{id}/commissioning` | Read commissioning-window and General Commissioning state |
| GET | `/api/devices/{id}/fabrics` | Read Operational Credentials fabric inventory |
| GET | `/api/devices/{id}/network-commissioning` | Read Network Commissioning state |
| GET | `/api/devices/{id}/groups?endpoint={endpoint}` | Read Groups state for one application endpoint |
| GET | `/api/devices/{id}/group-keys` | Read Group Key Management state from endpoint 0 |
| GET | `/api/devices/{id}/scenes?endpoint={endpoint}` | Read Scenes Management state for one application endpoint |
| POST | `/api/devices/{id}/network-commissioning/scan` | Scan visible Wi-Fi / Thread networks |
| POST | `/api/devices/{id}/network-commissioning/wifi` | Add or update one Wi-Fi network |
| POST | `/api/devices/{id}/network-commissioning/thread` | Add or update one Thread network |
| POST | `/api/devices/{id}/network-commissioning/connect` | Connect to one configured network |
| POST | `/api/devices/{id}/network-commissioning/remove` | Remove one configured network |
| POST | `/api/devices/{id}/network-commissioning/reorder` | Reorder one configured network |
| POST | `/api/devices/{id}/network-commissioning/interface-enabled` | Enable or disable the network interface |
| POST | `/api/devices/{id}/groups/add` | Add one group to an application endpoint |
| POST | `/api/devices/{id}/groups/view` | View one group on an application endpoint |
| POST | `/api/devices/{id}/groups/membership` | Read group membership from an application endpoint |
| POST | `/api/devices/{id}/groups/remove` | Remove one group from an application endpoint |
| POST | `/api/devices/{id}/groups/remove-all` | Remove all groups from an application endpoint |
| POST | `/api/devices/{id}/groups/add-if-identifying` | Add one group only if the endpoint is identifying |
| POST | `/api/devices/{id}/group-keys/write` | Write one group key set on endpoint 0 |
| POST | `/api/devices/{id}/group-keys/read` | Read one group key set from endpoint 0 |
| POST | `/api/devices/{id}/group-keys/remove` | Remove one group key set from endpoint 0 |
| POST | `/api/devices/{id}/group-keys/read-all-indices` | Read all group key set identifiers from endpoint 0 |
| POST | `/api/devices/{id}/scenes/add` | Add one scene to an application endpoint |
| POST | `/api/devices/{id}/scenes/view` | View one scene on an application endpoint |
| POST | `/api/devices/{id}/scenes/remove` | Remove one scene from an application endpoint |
| POST | `/api/devices/{id}/scenes/remove-all` | Remove all scenes for one group from an application endpoint |
| POST | `/api/devices/{id}/scenes/store` | Store one scene on an application endpoint |
| POST | `/api/devices/{id}/scenes/recall` | Recall one scene on an application endpoint |
| POST | `/api/devices/{id}/scenes/membership` | Read scene membership from an application endpoint |
| POST | `/api/devices/{id}/scenes/copy` | Copy one scene or all scenes on an application endpoint |
| POST | `/api/devices/{id}/commissioning/window/basic` | Open a basic commissioning window |
| POST | `/api/devices/{id}/commissioning/window/enhanced` | Open an enhanced commissioning window |
| POST | `/api/devices/{id}/commissioning/revoke` | Revoke the current commissioning window |
| POST | `/api/devices/{id}/commissioning/complete` | Send `CommissioningComplete` |
| POST | `/api/devices/{id}/fabrics/label` | Update the current fabric label |
| DELETE | `/api/devices/{id}/fabrics/{fabricIndex}` | Remove one fabric |
...
| DELETE | `/api/devices/{id}` | Remove device (remote fabric removal by default; `localOnly=true` for local cleanup only) |
...
Two controller services run on the Pi, **only one runs at a time** (systemd `Conflicts=` auto-switches):
...
- `DotMatter.Ui` is a separate companion service with its own deploy target: `dotnet msbuild DotMatter.Ui -t:Deploy`
- the UI service is `dot-matter-ui`, deployed under `/opt/dot-matter-ui`, and reads optional overrides from `/etc/dotmatter/dot-matter-ui.env`
- the UI defaults to `http://0.0.0.0:5001` and points at the controller through `ControllerApi__BaseUrl` (default `http://127.0.0.1:5000`)
- page-level UI behavior, screenshots, and operator routing live in [UI Guide](UI.md)
...
## Commissioning and Fabric Administration

The controller now surfaces the promoted Core administration helpers from `DotMatter.Core.MatterAdministration` on device-scoped endpoints.

- `GET /api/devices/{id}/commissioning` reads commissioning-window state plus the General Commissioning T&C/basic commissioning attributes from endpoint `0`.
- `POST /api/devices/{id}/commissioning/window/basic` opens a basic commissioning window with a JSON body such as `{ "commissioningTimeout": 180 }`.
- `POST /api/devices/{id}/commissioning/window/enhanced` opens an enhanced commissioning window using the Core `MatterAdministration.OpenCommissioningWindowAsync(...)` flow. The request body supplies the explicit Matter parameters:
  - `commissioningTimeout`
  - `pakePasscodeVerifierHex`
  - `discriminator`
  - `iterations`
  - `saltHex`
...
```http
POST /api/devices/device-id/commissioning/window/enhanced
X-API-Key: replace-with-a-long-random-value
Content-Type: application/json

{
  "commissioningTimeout": 180,
  "pakePasscodeVerifierHex": "00112233445566778899AABBCCDDEEFF00112233445566778899AABBCCDDEEFF00112233445566778899AABBCCDDEEFF00112233445566778899AABBCCDDEEFF",
  "discriminator": 3840,
  "iterations": 1000,
  "saltHex": "00112233445566778899AABBCCDDEEFF"
}
```
- `POST /api/devices/{id}/commissioning/revoke` revokes any open commissioning window.
- `POST /api/devices/{id}/commissioning/complete` sends `CommissioningComplete`.
- `GET /api/devices/{id}/fabrics` reads Operational Credentials fabric descriptors, NOCs, and trusted roots.
- `POST /api/devices/{id}/fabrics/label` updates the current fabric label with a JSON body such as `{ "label": "kitchen-fabric" }`.
- `DELETE /api/devices/{id}/fabrics/{fabricIndex}` removes one fabric from the target node.

These endpoints follow the same error model as the existing device reads: `404` for an unknown device, `503` for disconnected or timed-out devices, and `502` for transport/protocol failures.
...
> **Important:** The tracked base config requires an API key but does not include one. Production startup fails closed until `Controller__Security__ApiKey` is set. For trusted development or lab use, explicitly set `Controller__Security__RequireApiKey=false`.
...
For lab/testing mode, disable API-key enforcement explicitly:

```dotenv
Controller__Security__RequireApiKey=false
```
...
In the shipped `DotMatter.Ui` experience, the layout consumes `/api/events` for controller-feed status/toasts/history, while the dedicated `/live-feed` page listens to `/api/matter/events`.
