...
public class ClusterSupportPolicyTests
{
    ...
    [Test]
    public void CodeGen_StructAttribute_UsesTypedStructReader()
    {
        var cluster = new ClusterModel
        {
            Name = "Demo Cluster",
        ...
        Assert.That(code, Does.Contain("public Task<ApplicationStruct?> ReadCurrentAppAsync(CancellationToken ct = default)"));
        Assert.That(code, Does.Contain("=> ReadStructAttributeAsync(0x0001, raw => ReadApplicationStruct(raw, 2), ct);"));
        Assert.That(code, Does.Not.Contain("public Task<object?> ReadCurrentAppAsync"));
    }
    ...
    public void CodeGen_GlobalScalarAliases_MapToNumericTypes()
    {
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(Naming.MapCSharpType("power_mvar", false, false, false, null), Is.EqualTo("long"));
            ...
            Assert.That(Naming.MapCSharpType("int24u", false, false, false, null), Is.EqualTo("uint"));
            Assert.That(Naming.MapCSharpType("int24s", false, false, false, null), Is.EqualTo("int"));
        }
    }

    [Test]
    public void CodeGen_ClusterScopedTopLevelStructs_AreMergedAcrossXmlFiles()
    {
        var baseXml = File.ReadAllText(FindCodeGenXmlPath("operational-state-cluster.xml"));
        var derivedXml = File.ReadAllText(FindCodeGenXmlPath("operational-state-rvc-cluster.xml"));

        var mergedClusters = new Dictionary<uint, ClusterModel>();
        foreach (var cluster in ZapXmlParser.ParseAll(baseXml).Concat(ZapXmlParser.ParseAll(derivedXml)))
        {
        ...
        var rvcCluster = mergedClusters[0x0061];
        var code = ClusterCodeEmitter.Emit(rvcCluster, "operational-state-rvc-cluster.xml (+operational-state-cluster.xml)");

        using (Assert.EnterMultipleScope())
        {
            Assert.That(rvcCluster.Structs.Select(s => s.Name), Does.Contain("OperationalStateStruct"));
            Assert.That(rvcCluster.Structs.Select(s => s.Name), Does.Contain("ErrorStateStruct"));
            Assert.That(code, Does.Contain("public Task<OperationalStateStruct[]?> ReadOperationalStateListAsync(CancellationToken ct = default)"));
            Assert.That(code, Does.Contain("=> ReadArrayAttributeAsync(0x0003, ReadOperationalStateStruct, ct);"));
            Assert.That(code, Does.Contain("public Task<ErrorStateStruct?> ReadOperationalErrorAsync(CancellationToken ct = default)"));
            Assert.That(code, Does.Contain("=> ReadStructAttributeAsync(0x0005, raw => ReadErrorStateStruct(raw, 2), ct);"));
            Assert.That(code, Does.Contain("public ErrorStateStruct ErrorState { get; set; } = default!;"));
            Assert.That(code, Does.Not.Contain("public Task<object?> ReadOperationalErrorAsync"));
            Assert.That(code, Does.Not.Contain("requires parser support for ErrorState (object)."));
        }
    }
