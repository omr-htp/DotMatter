...
{
    private sealed record SentAnswer(Message Message, IPEndPoint EndPoint);
    ...
    private sealed class FakeMulticastService : IMulticastService
    {
        ...
        public List<SentAnswer> SentAnswers { get; } = [];
        ...
        public void SendAnswer(Message answer, bool checkDuplicate = true, IPEndPoint? unicastEndpoint = null)
            => SentAnswers.Add(new SentAnswer(PrepareAnswer(answer), unicastEndpoint ?? new IPEndPoint(IPAddress.Loopback, 5353)));

        public void SendAnswer(Message answer, MessageEventArgs query, bool checkDuplicate = true, IPEndPoint? endPoint = null)
            => SentAnswers.Add(new SentAnswer(PrepareAnswer(answer), endPoint ?? query.RemoteEndPoint));
