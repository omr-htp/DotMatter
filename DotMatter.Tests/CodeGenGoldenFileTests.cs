...
public class CodeGenGoldenFileTests
{
    ...
    [Test]
    public void TimedInvokeCommands_GenerateTimedCommandCalls()
    {
        var xmlPath = FindCodeGenPath("Xml", "administrator-commissioning-cluster.xml");
        var xml = File.ReadAllText(xmlPath);
        var cluster = ZapXmlParser.ParseAll(xml).Single(c => c.Id == 0x003Cu);
        var generated = ClusterCodeEmitter.Emit(cluster, "administrator-commissioning-cluster.xml");

        using (Assert.EnterMultipleScope())
        {
            Assert.That(generated, Does.Contain("public Task<InvokeResponse> OpenBasicCommissioningWindowAsync("));
            Assert.That(generated, Does.Contain("ushort timedTimeoutMs = 5000"));
            Assert.That(generated, Does.Contain("=> InvokeTimedCommandAsync(0x0001"));
            Assert.That(generated, Does.Contain("public Task<InvokeResponse> RevokeCommissioningAsync(CancellationToken ct = default, ushort timedTimeoutMs = 5000)"));
            Assert.That(generated, Does.Contain("=> InvokeTimedCommandAsync(0x0002, timedTimeoutMs: timedTimeoutMs, ct: ct);"));
        }
    }
