...
public class MatterTLVTests
{
    ...
    public void MessageDecode_ValidPayload_CreatesMessagePayload()
    {
        ...
        _ = new MessagePayload(messageFrameParts.MessagePayload);
    }

    [Test]
    public void MessageDecode_SecondValidPayload_CreatesMessagePayload()
    {
    ...
    [Test]
    public void MessageDecode_MinimalPayload_CreatesMessagePayload()
    {
    ...
    [Test]
    public void DecodeCertificatePayload_ValidTlv_RendersContent()
    {
        ...
        MatterTLV tlv = new(payloadAsBytes);

        Assert.That(tlv.ToString(), Is.Not.Empty);
    }

    [Test]
    public void DecodeEncryptedPasePayload_ValidPayload_ReadsMessageHeader()
    {
        ...
        var session = new PaseSecureSession(new FakeConnection(), sessionId, (ushort)0, StringToByteArray(encryptKey), StringToByteArray(decryptKey));
        ...
        {
            Assert.That(messageFrame.MessageFlags.GetDsiz(), Is.EqualTo(1));
            Assert.That((messageFrame.MessageFlags & MessageFlags.S), Is.EqualTo(MessageFlags.None));
    ...
    [Test]
    public void ReportDataPayload_ValidTlv_RendersContent()
    {
        ...
        MatterTLV tlv = new(payload);

        Assert.That(tlv.ToString(), Is.Not.Empty);
    }

    [Test]
    public void InvokeResponsePayload_ValidTlv_RendersContent()
    {
    ...
    [Test]
    public void AddTrustedRootCertificateRequestPayload_ValidTlv_RendersContent()
    {
    ...
    [Test]
    public void EncodeCertificatePayload_BasicFields_RendersContent()
    {
        ...
        encodedNocCertificate.AddOctetString(4, []); // subject-key-id
        encodedNocCertificate.AddOctetString(5, []); // authority-key-id
        ...
        Assert.That(encodedNocCertificate.ToString(), Is.Not.Empty);
    }

    [Test]
    public void EncodeUtf8Fields_SubjectList_RendersContent()
    {
        var encodedNocCertificate = new MatterTLV();
        encodedNocCertificate.AddStructure();
        encodedNocCertificate.AddList(6); // Subject
    ...
    [Test]
    public void ReadEncodedUtf8Fields_SubjectList_FindsExpectedTags()
    {
        var encodedNocCertificate = new MatterTLV();
        encodedNocCertificate.AddStructure();
        encodedNocCertificate.AddList(6); // Subject
        ...
        Assert.That(hasTag1, Is.False, "Expected no tag 1 at this point.");

        var hasTag6 = encodedNocCertificate.IsNextTag(6);
        Assert.That(hasTag6, Is.True, "Expected tag 6 at this point.");
