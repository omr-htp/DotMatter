...
public class Spake2PlusTests
{
    ...
    public void Test_Spake2Plus_FullHandshake()
    {
        ...
        _ = pakeValues.X.GetEncoded(false);

        // Responder (Device)
        BigInteger y = new(RandomNumberGenerator.GetBytes(32), isUnsigned: true, isBigEndian: true);
        var Y_point = P256Math.Add(P256Math.Multiply(P256Math.G, y), P256Math.Multiply(P256Math.N_Point, pakeValues.W0));
        byte[] Y_bytes = Y_point.GetEncoded(false);

        // Initiator processes Sigma2 (Y)
        var p2 = CryptographyMethods.Crypto_P2(
            contextHash,
            pakeValues.W0,
            pakeValues.W1,
            pakeValues.XScalar,
        ...
        Assert.That(p2.Ke, Is.Not.Null);
        using (Assert.EnterMultipleScope())
        {
            Assert.That(p2.Ke, Has.Length.EqualTo(16));
            Assert.That(p2.HAY, Has.Length.EqualTo(32));
            Assert.That(p2.HBX, Has.Length.EqualTo(32));
        }
    }
}
